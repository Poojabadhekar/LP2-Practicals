...
bool isSafe(int row, int col, int slash[N][N], int backSlash[N][N], 
            bool rowLookup[], bool slashLookUp[], bool backSlashLookUp[]) {
    if (slashLookUp[slash[row][col]] || 
        backSlashLookUp[backSlash[row][col]] || 
        rowLookup[row]) {
        return false;
    }
    return true;
}
bool solveNqueenUtil(int board[N][N], int col, int slash[N][N], int backSlash[N][N], 
                     bool rowLookUp[N], bool slashLookUp[N], bool backSlashLookUp[N]) {
    ...
    for (int i = 0; i < N; i++) {
        if (isSafe(i, col, slash, backSlash, rowLookUp, slashLookUp, backSlashLookUp)) {
            ...
            if (solveNqueenUtil(board, col + 1, slash, backSlash, rowLookUp, slashLookUp, backSlashLookUp))
                return true;
            board[i][col] = 0;
            rowLookUp[i] = false;
